...
### Tasks
- [x] Implement daily session state model with versioning
- [x] Create session fields: user_id, guest_id, puzzle_date_key, puzzle_id, guesses, hint_flags, completion_state, version, timestamps
- [x] Build guess submission endpoint with session version validation
- [x] Implement hint unlock rules (after guess 3, 6, 9) server-side
- [x] Create completion detection: solved, 10 guesses used, or missed after day rollover
- [x] Implement stale-device detection and rejection with messaging
- [x] Build daily session refresh endpoint
- [x] Implement hard-lock on completed games (no further input allowed)
- [x] Create compact daily_results archival after session completion
- [x] Build guest-to-account migration with same-day session transfer

### Acceptance Criteria
- [x] Guess submission validates against server state
- [x] Hints unlock only at correct guess counts
- [x] Session version prevents stale-device overwrites
- [x] Completed games cannot be modified
- [x] Stale-device errors display without silent refresh
- [x] Guest-to-account migration transfers today's progress only
- [x] Missed day after rollover counts as both missed participation and loss
- [ ] Guess rollback happens when submission is rejected by server (frontend — deferred to Phase 8)
...
### Tasks
- [x] Implement stats tracking: total_participations, total_wins, total_losses, win_rate
- [x] Build win streak tracking (increments on correct daily answer, breaks on loss or missed day)
- [x] Build participation streak tracking (increments on any guess that day, breaks only on missed day, not on loss)
- [x] Implement best win streak and best participation streak tracking
- [x] Create solved-in-X counters (solved_in_1 through solved_in_10)
- [x] Build average-guesses-to-win calculation (winning games only)
- [x] Create best-guess summary display logic
- [x] Implement stats denormalization/precomputation in user_stats table
- [x] Build dashboard stats endpoint returning all tracked metrics

### Acceptance Criteria
- [x] Win streak increments on correct answer and breaks correctly
- [x] Participation streak increments on any participation and breaks only on missed days
- [x] Win rate correctly calculates wins/participated_days
- [x] Best guess summary displays "Solved in X guesses: Y times" format
- [x] Stats persist correctly across device refreshes
- [x] Unverified users do not contribute to stats until verification
